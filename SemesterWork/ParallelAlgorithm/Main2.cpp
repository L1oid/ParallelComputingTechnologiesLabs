...
int main(int argc, char* argv[])
{
	...
	MPI_Allreduce(&min_distance, &global_min_distance, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

	if (rank == 0)
	{
		if (min_distance != global_min_distance)
		{
			MPI_Recv(min_way, n, MPI_INT, i, 777, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
	}
	...
	if (rank == 0)
	{
		...
		if (size < n)
		{
			final_way = new int* [size - 1];
			final_way[0] = new int[(size - 1) * n];
